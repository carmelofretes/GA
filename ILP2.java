...
//        hacer el doble for para las k y solicitudes
        ...
        for (int i = 0; i < kTotales; i++) {
            for (int j = 0; j < cantSolicitadas.size(); j++) {
                String path = "C:\\Users\\ysapymimbi\\IdeaProjects\\tesis\\src\\ga\\archivos\\6nodos\\"+ algoritmo +"\\cargaUniforme\\k" + (i + 1) + 
                        "\\cantSolicitada" + (cantSolicitadas.get(j)) + "\\";
                String solicitudesPath = path + "solicitudes.txt";
                
                // esto es para los parametros del ag
                String salidaPath = path + "ga.txt";
                ...
                if ("ilp".equals(algoritmo)) {
                    ...
                    String saltos = leerSaltos(path);
                    List<String> maximos = leerMaximos(path);

                    String pathILP= "C:\\Users\\ysapymimbi\\IdeaProjects\\tesis\\src\\ga\\archivos\\6nodos\\ilp\\cargaUniforme\\k" + (i + 1) + 
                            "\\cantSolicitada" + (cantSolicitadas.get(j)) + "\\";
    ...
    //                Probamos la conexión de Java con CPLEX 
                    String argumentos[] = {"-v", "C:\\Users\\ysapymimbi\\opl\\ILP2_3\\ILP2_3.mod", pathILP + "ILP2_3.dat", pathILP + "salidaCplexILP2_3.txt", "1800"};
